...
//  Blocstagram
//
//  Created by Dorian Kusznir on 4/29/15.
...
- (void)addFiltersToQueue
{
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        CIFilter *pixelFilter = [CIFilter filterWithName:@"CIPicxellate"];
    ...
    //First Filter - Chrome Effect:
    
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        CIFilter *accordionFilter = [CIFilter filterWithName:@"CIPhotoEffectChrome"];
        
        if (accordionFilter)
        {
            [accordionFilter setValue:sourceCIImage forKey:kCIInputImageKey];
            [self addCIImageToCollectionView:accordionFilter.outputImage withFilterTitle:NSLocalizedString(@"Chrome", @"Chrome Filter")];
        }
    }];
    
    //Compound Filter - Vignette Effect:
    
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        CIFilter *vignetteFilter = [CIFilter filterWithName:@"CIVignetteEffect" keysAndValues:kCIInputImageKey, sourceCIImage,
        ...
        if (vignetteFilter)
        {
            ...
            [self addCIImageToCollectionView:vignetteFilter.outputImage withFilterTitle:NSLocalizedString(@"Vignette Effect", @"Vignette Effect Filter")];
        }
    }];
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        ...
        CIFilter *darkScratches = [CIFilter filterWithName:@"CIColorMatrix" keysAndValues:kCIInputImageKey, otherRandomImage,
                                   ...
                                   @"inputRVector", [CIVector vectorWithX:0.0 Y:0.0 Z:0.0 W:0.0],
                                   @"inputRVector", [CIVector vectorWithX:0.0 Y:0.0 Z:0.0 W:0.0],
                                   @"inputRVector", [CIVector vectorWithX:0.0 Y:0.0 Z:0.0 W:0.0],
                                   @"inputRVector", [CIVector vectorWithX:0.0 Y:1.0 Z:1.0 W:1.0],
                                   nil];
...
- (void)sendButtonPressed:(id)sender
{
    ...
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"LOL" message:NSLocalizedString(@"Add a caption and send your image to Instagram.", @"Send image instructions.") delegate:self cancelButtonTitle:NSLocalizedString(@"Cancel", @"Cancel Button") otherButtonTitles:NSLocalizedString(@"Send", @"Send Button"), nil];
    ...
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"No Instagram App", nil) message:NSLocalizedString(@"Instagram isn't installed on your device. Please visit the App Store to install Instagram.", nil) delegate:nil cancelButtonTitle:NSLocalizedString(@"OK", @"OK Button") otherButtonTitles:nil];
        [alert show];
    }
}
